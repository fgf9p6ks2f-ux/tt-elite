# Table tennis — actionable bets (TT Elite · Setka · Liga Pro · TT Cup)

_updated 2026-07-20 10:46 UTC_

Per-league shrunk-posterior rules (holdout-validated). Fewer, sharper bets.

```

498 upcoming fixtures (all leagues) · 246,293 historical matches · line 74.5 · per-league validated rules · 2 new alert(s)

=== 10 ACTIONABLE BETS TODAY ===
  when            league      matchup                                       zone  conf   raw    n    avg
  Mon 2:35PM MT   TTCup·shadowRichard Vavricka vs Jan Vidourek            O≤77.5   78%   88%   24   86.2
  Mon 5:00AM MT   Elite       Pawel Adamus vs Oliver Vincenec              O78.5   77%   77%   13   86.2
  Mon 6:10AM MT   Setka·shadowVladimir Voronenko vs Artem Yurkov          O≤77.5   76%   89%   18   86.2
  Mon 10:10AM MT  Setka·shadowArtem Yurkov vs Vladyslav Mishchenko        O≤76.5   74%  100%    9   84.7
  Mon 5:20AM MT   TTCup·shadowDavid Sykora vs Vaclav Hejda Jr             O≤77.5   74%   92%   12   83.9
  Mon 12:55PM MT  TTCup·shadowZbynek Vyskocil vs Christian Kratochvil     O≤77.5   73%   87%   15   88.9
  Mon 4:30AM MT   Elite       Jakub Chmielowski vs Krystian Kolodziej      O60.5   70%   70%   37   71.4
  Mon 9:40AM MT   TTCup·shadowBruno Mrowetz vs Jan Skvrna                 O≤77.5   70%   79%   19   87.5
  Mon 10:00AM MT  TTCup·shadowWiekiera Arkadiusz vs Jakub Komorowicz      O≤75.5   69%   74%   31   84.5
  Mon 5:25AM MT   TTCup·shadowMartin Filipek vs Frantisek Balcar          O≤77.5   68%   82%   11   97.1

'zone' = bettable line range: O≤X take the Over at any posted total up to X; U≥Y take the Under at any total from Y up; posted line outside the zone = skip. 'conf' = league rule confidence; 'raw' = H2H side rate at 74.5; 'avg' = average total.

```
